...
## Estado atual do projeto (17/05/2026)
...
| Workflow newsletter (07:50 BRT) | Ativo, aguardando confirmação de email |
| Workflow Instagram | Ativo |
| Supabase `relatorios` | Relatórios diários com schema v3 |
| Supabase `newsletter_subscribers` | Tabela criada com RLS |
| Supabase `comunicados` | Tabela criada |
| Dashboard Vercel | Dark/light mode, gauge, sparkline, IG 2 por linha |
| SMTP de teste | Resend configurado no n8n (`onboarding@resend.dev`) |
| SMTP Fotus (corporativo) | Pendente — requer chamado ao TI |
...
### Imediatos
1. **Confirmar chegada do email** de teste em `igor.pirola@fotus.com.br`
2. **Verificar execução do workflow de análise diária** com os headers corrigidos
3. **Liberar SMTP Fotus** via TI — substituir credencial Resend pela corporativa
4. **Compartilhar URL do dashboard** com Thiago para teste

### Médio prazo
5. **WhatsApp para diretores** — adicionar nó Evolution API no workflow de newsletter para enviar briefing texto direto no WhatsApp (precisa de URL + instância da Evolution API)
6. **Deletar registro 2099-01-01** no Supabase (requer service role key)
7. **Resolver cobertura de grupos** — entender quais grupos o bot Z-API monitora

### Longo prazo
8. **Histórico em calendário** — visão de 30 dias com indicador de temperatura
9. **Alertas proativos** — delta > 15 pontos dispara notificação WhatsApp
10. **Relatório semanal** gerado automaticamente a partir dos 7 diários
...
## ERRO 12 — Inscrição na newsletter não salvava no banco

**Fase:** 11 | **Data:** 17/05/2026

### O que aconteceu
Botão "Inscrever-se" mostrava sucesso mas não registrava o email no Supabase — era apenas validação client-side sem chamada ao banco.

### Causa
A função `submitSubscription()` foi implementada sem o INSERT no Supabase, apenas validando o domínio `@fotus.com.br`.

### Solução
Adicionar `sb.from('newsletter_subscribers').upsert({ email, ativo: true }, { onConflict: 'email', ignoreDuplicates: true })`.

### Aprendizado
Features de captação (inscrição, formulário, CTA) devem ser testadas de ponta a ponta imediatamente após implementação — não confiar que "parece funcionar" na UI.
...
| Feature incompleta | 12 (inscrição sem INSERT no banco) |
| Configuração n8n | 14 (Client Host Name inválido), 15 (placeholder em produção), 16 (env vars) |
